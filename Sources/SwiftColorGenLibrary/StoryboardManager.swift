...
public struct StoryboardManager {
    ...
    public static func updateStoryboard(path: String, colors: Set<ColorData>) -> AEXMLDocument {
        guard let data = try? Data(contentsOf: URL(fileURLWithPath: path)) else {
            return AEXMLDocument()
        }
        guard let xml = try? AEXMLDocument(xml: data, options: AEXMLOptions()) else {
    ...
    public static func addResources(xml: AEXMLDocument, colors: Set<ColorData>) {
    ...
    public static func removeOriginalResources(xml: AEXMLDocument, assets: [Asset]) {
        guard let resources = xml.root.getChild(name: "resources") else {
            return
        }
        ...
        resources.children.forEach { $0.removeFromParent() }
        resources.addChildren(valid)
    }
    
    // Updating the resources, for custom colors
    public static func updateCustomResources(xml: AEXMLDocument, assets: [Asset]) {
        guard let resources = xml.root.getChild(name: "resources") else {
            return
        }
    ...
    public static func resetColors(xml: AEXMLDocument, assets: [Asset]) {
        ColorManager.resetColors(xml: xml.root, assets: assets)
    }
    
    // Write the updated storyboard to file
    public static func writeStoryboard(xml: AEXMLDocument, path: String) {
